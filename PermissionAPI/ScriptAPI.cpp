...
public:
    ...
    static PermGroupClass* constructor(const Arguments& args) {
        ...
        try {
            return new PermGroupClass(args.thiz(), mod.perm.createGroup(name, displayName));
        }
        CATCH_AND_THROW;
        return nullptr;
    }
...
public:
    ...
    static Local<Value> groupExists(const Arguments& args) {
        ...
        try {
            return Boolean::newBoolean(mod.perm.groups.contains(args[0].toStr()));
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> getGroup(const Arguments& args) {
        ...
        try {
            return PermGroupClass(args.thiz(), mod.perm.groups.at(args[0].toStr())).getScriptObject();
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> getOrCreateGroup(const Arguments& args) {
        ...
        try {
            return PermGroupClass(args.thiz(), mod.perm.getOrCreateGroup(args[0].toStr())).getScriptObject();
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> registerAbility(const Arguments& args) {
        ...
        try {
            mod.perm.registerAbility(args[0].toStr(), args[1].toStr());
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> deleteAbility(const Arguments& args) {
        ...
        try {
            mod.perm.deleteAbility(args[0].toStr());
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> abilityExists(const Arguments& args) {
        ...
        try {
            return Boolean::newBoolean(mod.perm.abilitiesInfo.contains(args[0].toStr()));
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }

    static Local<Value> checkAbility(const Arguments& args) {
        CHECK_ARGS_COUNT(1);
        CHECK_ARG_TYPE(0, kString);
        CHECK_ARG_TYPE(1, kString);

        try {
            return Boolean::newBoolean(mod.perm.checkAbility(args[0].toStr(), args[1].toStr()));
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> isMemberOf(const Arguments& args) {
        ...
        try {
            return Boolean::newBoolean(mod.perm.isMemberOf(args[0].toStr(), args[1].toStr()));
        }
        CATCH_AND_THROW;
        return Local<Value>();
    }
    ...
    static Local<Value> getPlayerGroups(const Arguments& args) {
        ...
        try {
            auto groups = mod.perm.getPlayerGroups(args[0].toStr());
    ...
    static Local<Value> getPlayerAbilities(const Arguments& args) {
        ...
        try {
            auto abilities = mod.perm.getPlayerAbilities(args[0].toStr());
            auto res = Array::newArray();
...
ClassDefine<void> PermScriptAPIBuilder =
    defineClass("perm")
        ...
        .function("checkAbility", &PermissionClass::checkAbility)
